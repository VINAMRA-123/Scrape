...
import time
import csv
urls = ["https://finance.yahoo.com/quote/MSFT?p=MSFT&.tsrc=fin-srch","https://finance.yahoo.com/quote/AMZN?p=AMZN&.tsrc=fin-srch"]
headers = {'user-agent':'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/97.0.4692.71 Safari/537.36'}

csv_file = open('scrape1.py',"w")
csv_writer = csv.writer(csv_file)
csv_writer.writerow(['Stock Name', 'Current Price', 'Previous Close', 'Open', 'Bid', 'Ask', 'Day Range', '52 Week Range', 'Volume', 'Avg. Volume'])

for url in urls:
    stock = []
    html_page = requests.get(urls,headers= headers)
    soup = BeautifulSoup(html_page.content,'lxml')
    title = soup.find("title").get_text()  #here get_text is used for removing tags which is <title>
...
send_email.send(filename=today)